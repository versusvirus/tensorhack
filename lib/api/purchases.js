...
module.exports = function (app, db) {
        ...
        if (mode === 'multi') {
            const {rows} = await db.query(`WITH Prices AS (
                                  ...
                                  FROM
                                    products_shops P_S
                                  WHERE 
                                    P_S.product_id = ANY (SELECT _id FROM purchases WHERE note_id=${note_id})
                                  GROUP BY
                                    P_S.product_id
                                ) 
    ...
    app.post('/purchases', async (req, res) => {
        const {note_id, product_name} = {...req.body};

        const [{_id: product_id}] = (await db.query(`SELECT _id FROM public.products WHERE name='${product_name}'`)).rows;
        await db.query(`INSERT INTO public.purchases ("note_id", "product_id") 
                        SELECT ${note_id},${product_id} FROM public.purchases
                        WHERE NOT EXISTS(SELECT 1 FROM public.purchases WHERE product_id=${product_id})`);
