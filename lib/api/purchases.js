    ...
    app.get('/purchases', async (req, res) => {
        ...
        if (mode === 'multi') {
                const {rows} = await db.query(
                        ...
                        and purchase_characteristic.value = product_characteristic.value and purchases.note_id = ${note_id}
                        
                        group by purchases.category_id
                        )
                        
                        select products_shops.price as price, products.name as product_name, products.logo as product_logo, coalesce(shops.name, 'Not in sale') as shop_name from prices
